...
<div class="card border-0 shadow-sm rounded-4 bg-white" style="max-width: 600px;">
    <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
        <h5 class="mb-0 fw-bold"><i class="bi bi-pencil me-2"></i> Chỉnh sửa Album ảnh</h5>
    ...
    <div class="card-body p-4">
        ...
        <form action="<?= base_url('admin/gallery-albums/update/' . $album['id']) ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="sort_order" name="sort_order" value="<?= old('sort_order', $album['sort_order']) ?>" required>
                </div>
                
                <div class="col-md-6">
                    <label for="status" class="form-label fw-semibold">Trạng thái hiển thị <span class="text-danger">*</span></label>
                    <select class="form-select" id="status" name="status" required>
                        <option value="1" <?= old('status', $album['status']) == '1' ? 'selected' : '' ?>>Hiển thị</option>
                        <option value="0" <?= old('status', $album['status']) == '0' ? 'selected' : '' ?>>Ẩn</option>
                    </select>
                </div>
            </div>
            
            <div class="text-end">
                <button type="submit" class="btn btn-primary btn-custom px-5 rounded-pill"><i class="bi bi-save-fill"></i> Cập Nhật Album</button>
            </div>
        </form>
    </div>
</div>
