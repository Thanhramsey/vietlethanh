...
<div class="about-hero-banner">
    ...
    <div class="about-hero-wave">
        <svg viewBox="0 0 1440 80" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
            <path fill="currentColor" class="text-bg-wave" d="M0,40 C240,80 480,0 720,40 C960,80 1200,10 1440,40 L1440,80 L0,80 Z"/>
        </svg>
    </div>
</div>
            ...
            <div class="col-lg-6" data-aos="fade-right" data-aos-duration="900">
                <div class="about-img-wrapper">
                    ...
                    <div class="about-img-years">
                        <span class="display-4 fw-black text-primary"><?= esc($statExp) ?></span>
...
<section class="section-padding bg-light-gray" id="about-timeline-section" style="overflow: hidden;">
    <div class="container">
        <div class="text-center mb-5" data-aos="fade-up">
            ...
            <p class="text-muted max-width-600 mx-auto">Nhấp chọn các mốc thời gian bên dưới để xem chi tiết hành trình phát triển qua các giai đoạn nổi bật.</p>
        </div>

        <div class="timeline-wrapper position-relative" data-aos="fade-up" data-aos-duration="1000">
            <!-- Timeline Line background -->
            <div class="timeline-line"></div>

            <!-- Timeline Year Buttons (Navigation) -->
            <div class="timeline-navigation d-flex justify-content-between align-items-center mb-5 position-relative" style="z-index: 5;">
                <?php foreach ($milestones as $index => $item): ?>
                    <button class="timeline-year-btn <?= $index === 0 ? 'active' : '' ?>" 
                            data-target="milestone-<?= esc($item['id']) ?>">
                        <span class="timeline-year-number"><?= esc($item['year']) ?></span>
                        <span class="timeline-year-dot"></span>
                    </button>
                <?php endforeach; ?>
            </div>

            <!-- Timeline Slides (Contents) -->
            <div class="timeline-slides-container card border-0 shadow-lg rounded-4 overflow-hidden" style="min-height: 320px;">
                <div class="card-body p-0">
                    <?php foreach ($milestones as $index => $item): ?>
                        <div class="timeline-slide <?= $index === 0 ? 'active' : '' ?>" id="milestone-<?= esc($item['id']) ?>">
                            <div class="row g-0 align-items-stretch h-100">
                                <div class="col-lg-6">
                                    <div class="timeline-slide-img-box h-100">
                                        <?php if (!empty($item['image'])): ?>
                                            <img src="<?= base_url('uploads/milestones/' . $item['image']) ?>" alt="<?= esc($item['title']) ?>" class="timeline-slide-img">
                                        <?php else: ?>
                                            <div class="timeline-slide-img-fallback d-flex flex-column align-items-center justify-content-center h-100">
                                                <i class="bi bi-clock display-3 text-primary mb-3" style="opacity: 0.5;"></i>
                                                <span class="fw-black text-primary display-5"><?= esc($item['year']) ?></span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="col-lg-6 d-flex align-items-center">
                                    <div class="p-4 p-md-5">
                                        <div class="timeline-slide-year-badge mb-3">
                                            <span class="badge bg-primary-subtle text-primary rounded-pill px-4 py-2 fs-6 fw-bold"><?= esc(lang('Site.milestone_year')) ?> <?= esc($item['year']) ?></span>
                                        </div>
                                        <h3 class="timeline-slide-title mb-3 fw-bold text-dark"><?= esc($item['title']) ?></h3>
                                        <p class="timeline-slide-desc text-muted mb-0 lead" style="font-size: 1rem; line-height: 1.85;">
...
<section class="section-padding" style="background: linear-gradient(135deg, #0f2044 0%, #1e3c72 50%, #2a5298 100%); position:relative; overflow:hidden;">
    ...
    <div class="container position-relative" style="z-index:1;">
        ...
        <div class="row g-4">
            <?php foreach ($companyInfos as $i => $info): ?>
...
<!-- Page-specific Styles and Script for Timeline -->
<style>
/* ===== ABOUT HERO BANNER ===== */
.about-hero-banner {
    position: relative;
...
.sector-card h4 { font-weight: 700; margin-bottom: 14px; color: var(--text-color); }
.sector-card p  { color: var(--text-muted); font-size: 0.9rem; line-height: 1.75; margin-bottom: 20px; }
.sector-link {
    display: inline-flex;
    align-items: center;
    gap: 8px;
...
.timeline-year-btn.active .timeline-year-dot {
    background: var(--primary-color);
    border-color: var(--primary-color);
    box-shadow: 0 0 0 6px var(--bg-body), 0 0 16px rgba(11, 94, 215, 0.4);
...
<!-- Timeline Interactive Switcher Script -->
<script>
